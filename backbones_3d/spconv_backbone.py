...
class VoxelBackBone8xFusion(nn.Module):
    ...
    def point_fusion(self,
        ...
        def construct_multimodal_features(x_list,
            ...
            """
            # indices[N_v,4] (B,z,y,x)
            voxel_indices = (x_list[-1].indices)[:, 1:] #[:, [0, 2, 1]] # (N_v,3)(zyx->zxy)
            ...
            D_, H_, W_ = x_list[-1].spatial_shape
            ...
            for b in range(batch_size):
                ...
                voxel_indices_batch_offsets = voxel_indices_batch + torch.tensor([0.5, 0.5, 0.5]).cuda()
                voxel_indices_norm = voxel_indices_batch_offsets[:, [0, 2, 1]] / torch.tensor([D_, W_, H_], device=voxel_indices_batch.device)
                ...
                if "ACTR" in fusion_method:
                    ...
                    voxel_indices_list.append(voxel_indices_batch)
                    coor_3d_list.append(voxel_indices_norm)
            ...
            if "ACTR" in fusion_method:
                ...
                voxel_indices_b = torch.zeros(batch_size, self.max_num_nev, 3).cuda()
                coor_3d_b = torch.zeros(batch_size, self.max_num_nev, 3).cuda()
                for b in range(batch_size):
                    if False:
                    ...
                    voxel_indices_b[b, :voxel_indices_list[b].shape[0]] = voxel_indices_list[b]
                    coor_3d_b[b, :coor_3d_list[b].shape[0]] = coor_3d_list[b]
                ...
                enh_feat = self.actr(
                    ...
                    voxel_dense_feat=x_list[-1].dense(),
                    voxel_sparse_indices=voxel_indices_b[:, :n_max],
                    voxel_grid=coor_3d_b[:, :n_max]
                )
