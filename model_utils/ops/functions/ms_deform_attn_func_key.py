...
def ms_deform_attn_core_pytorch_key_aware(query, value, key, input_padding_mask, value_spatial_shapes,
                                          sampling_locations, key_proj, value_proj, query_proj, i_query_proj,
                                          attention_weights_linear, add, i_query=None):
    # for debug and test only,
    # need to use cuda version instead
    ...
    query = query.unsqueeze(-2) # N*M*Lq, 1, D

    if i_query is not None:
        i_query = i_query_proj(i_query)
        i_query = i_query.view(N_, Lq, M_, DD_ // M_)
    ...
    output = output.squeeze(-2).view(N_, M_, Lq_, D_).permute(0, 2, 1, 3)  # N*M*Lq, 1,  D ->N*M*Lq,D->N,M,Lq,D->N, Lq, M,  D
