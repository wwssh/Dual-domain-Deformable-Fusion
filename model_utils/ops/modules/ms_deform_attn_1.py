...
class MSDeformAttn(nn.Module):
    ...
    def forward(
        ...
        # if Dual_spatial:
        #     #[B,N_v,4,4,3]
        #     # query_positon_v = sampling_locations_v * offset_normalizer_v[None, None, None, :, : ]
        #
        #     # #value_v[B,N_v,8,8]
        ...
        #     # sampling_value_v = self.KDTree_nearest_search(query_positon_v, value_v, q_lidar_indices)#[B*8,N_v*4,8]
        #     # sampling_value_v = self.KDTree_nearest_search_multiprocess(query_positon_v, value_v, q_lidar_indices)  # [B*8,N_v*4,8]
        #
        #     # end_time = time.time()
        #     # e_time = end_time - start_time
        #     # print(f"单进程执行时间:{e_time}")
        #
        #     # sampling_value_v = sampling_value_v.transpose(1, 2).reshape(N_ * M_, D_, Lq_, P_)  # [B*8,8,N_v,4]
        #     # attention_weights_v = attention_weights_v.transpose(1, 2).reshape(N_ * M_, 1, Lq_, L_ * P_)  # [B*8,1,N_v,4]
        #     # 更新的体素特征
        #     # output_v = (sampling_value_v * attention_weights_v).sum(-1).view(N_, M_ * D_, Lq_).transpose(1, 2).contiguous() # [B,64,N_v]
        #     output_v = self.output_proj_v(output_v)  # [B,N_v,64]
        #     output_dict['output_v'] = output_v
