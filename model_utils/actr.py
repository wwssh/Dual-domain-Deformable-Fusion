...
class ACTR(nn.Module):
    ...
    def __init__(
        ...
        self.voxel_xy_position_embedding = PositionEmbeddingSineSparse(
            num_pos_feats=self.transformer.q_model // 2, normalize=True)
        self.voxel_D_positon_embedding = PositionEmbeddingSineSparseD(
            num_pos_feats=self.transformer.q_model, normalize=True)
    ...
    def forward(
        ...
        voxel_normindices_xy = voxel_grid[..., 1:]
        voxel_normindices_D = voxel_grid[..., 0]
        v_xy_pos = self.voxel_xy_position_embedding(voxel_normindices_xy).transpose(1, 2)
        # v_D_pos = self.voxel_D_positon_embedding(voxel_normindices_D).transpose(1, 2)
        # voxel_pos = v_xy_pos+v_D_pos
        ...
        q_enh_feats = self.transformer(srcs,
                                       ...
                                       voxel_dense_feat=voxel_dense_feat,
                                       voxel_sparse_indices=voxel_sparse_indices,
