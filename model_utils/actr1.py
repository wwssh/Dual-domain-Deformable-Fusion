...
from .actr_transformer1 import build_deformable_transformer
...
class ACTR(nn.Module):
    ...
    def forward(
        ...
        """Parameters:
            v_feat: 3d coord sparse voxel features (B, C, X, Y, Z)稀疏体素特征[B,N_v,64]
            grid: image coordinates of each v_features (B, X, Y, Z, 3)体素投影到图像坐标系(norm)[B,N_v,2] (x,y)
            grid_ld: 体素座标(norm)[B,N_v,3](xyz)
            i_feats: image features (consist of multi-level)图像特征list [B,256,h/4,w/4]
            v_i_feat: 每个体素投影后对应的图像特征[B,N_v,256]
            lidar_grid: 体素座标[B,N_v,3](xyz)
        ...
        if self.pos_encode_method == "image_coor":
            ...
            q_depths = lidar_grid[..., 0].clone()#采用x的值进行深度编码
        ...
        q_enh_feats = self.transformer(srcs,#降维图像特征 list [B,64,h/4,w/4]
                                       masks,#list [B,h/4,w/4]
                                       pos,#图像特征xy位置编码;list;[B,64,h/4,w/4]
                                       q_feat_flattens,#稀疏体素特征[B,N_v,64]
                                       q_pos,#深度编码[B,N_v,64]
                                       q_ref_coors,#体素投影到图像坐标系(norm)[B,N_v,2] (x,y)
                                       q_ref_ld_coors=grid_ld,#体素坐标(norm)[B,N_v,3] (xyz)
                                       q_lidar_grid=lidar_grid,#体素座标[B,N_v,3];(xyz)
                                       q_i_feat_flatten=q_i_feat_flattens,#稀疏体素投影图像特征[B,N_v,64]
                                       v_indices=v_indices,
...
def build(model_cfg, model_name='ACTR', lt_cfg=None, hybrid_cfg=None):
    ...
    args.dual_spatial = model_cfg.dual_spatial
