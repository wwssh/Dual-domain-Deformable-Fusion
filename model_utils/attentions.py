...
class GateFusion(nn.Module):
    def __init__(self, channel):
        super().__init__()
        self.conv1d = nn.Conv1d(channel*2, channel, kernel_size=1, stride=1, padding=0)
        self.conv1d_1 = nn.Conv1d(channel, channel, kernel_size=1, stride=1, padding=0)
        self.conv1d_2 = nn.Conv1d(channel, channel, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, feat1, feat2):
        ...
        '''
        feat1 = feat1.permute(0, 2, 1)#[B,C,N_v]
        feat2 = feat2.permute(0, 2, 1)
        concat_feat = torch.cat([feat1, feat2], dim=1)#[B,2C,N_v]
        feat_map = self.conv1d(concat_feat)#[B,C,N_v]
        feat_weight = torch.sigmoid(feat_map)#[B,C,N_v]
        fus_feat = feat1 * feat_weight + feat2 * (1 - feat_weight)#[B,C,N_v]

        feat1_weight = torch.sigmoid(self.conv1d_1(feat1 + fus_feat))
        ffeat1 = feat1 * feat1_weight + fus_feat * (1 - feat1_weight)

        feat2_weight = torch.sigmoid(self.conv1d_2(feat2 + fus_feat))
        ffeat2 = feat2 * feat2_weight + fus_feat * (1 - feat2_weight)

        return ffeat1.permute(0, 2, 1), ffeat2.permute(0, 2, 1)
